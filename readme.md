...
## 設定ファイル管理クラス (ConfigManager)

このプロジェクトは、アプリケーションの設定情報を永続的に管理し、その整合性をハッシュ値によって保証するためのPythonクラスを提供します。特に、一度作成されたら内容が意図せず変更されることを防ぎたい、ローカル環境での設定やセーブデータ管理に適しています。

### 主な機能

- **設定ファイルのロードまたは新規作成**: 指定されたパスに設定ファイル（JSON形式）が存在すれば読み込み、存在しなければデフォルト設定で新規作成します。
- **ハッシュ値による整合性チェック**: 設定ファイルの読み込み時に、対応するハッシュファイルに記録されたハッシュ値と比較し、ファイル内容が変更されていないかを確認します。
- **厳格な整合性保証**: 設定ファイルが存在するにも関わらずハッシュファイルが見つからない場合は、エラーとして扱います。また、ハッシュ値が一致しない場合もエラーを発生させ、不正な状態でのアプリケーション起動を防ぎます。
- **設定値の読み取り専用提供**: ロードされた設定データへのアクセス機能（設定値の取得）のみを提供し、ConfigManagerインスタンス経由での設定値の変更やファイルへの書き戻し機能は持ちません。
- **責務の分離**: 設定ファイルの読み書き (`ConfigLoader`) とハッシュ値の管理 (`HashManager`) を別のクラスに分離し、ConfigManagerがそれらを統合することで、コードの保守性と理解性を高めています。

### クラス構造

- **`ConfigLoader`**:
    - 設定ファイル（JSON）の読み込みとパース、およびデフォルト内容での新規作成を担当します。
    - ファイルの存在確認、読み込みエラー（IOError, JSONDecodeError）のハンドリングを行います。
- **`HashManager`**:
    - 文字列やバイト列のハッシュ値（SHA256）計算を担当します。
    - ハッシュ値をファイルに保存、またはファイルから読み込む機能を持ちます。
- **`ConfigManager`**:
    - `ConfigLoader` と `HashManager` のインスタンスを内部に持ち、全体のワークフローを制御します。
    - 初期化時に設定ファイルのロード/作成、現在のハッシュ計算、保存されたハッシュのロード、両者の比較による整合性チェックを行います。
    - 整合性が確認された設定データへの読み取り専用アクセスを提供します。

### 使用方法

`ConfigManager` クラスのインスタンスを作成するだけで、指定されたパスから設定ファイルのロードまたは新規作成、および整合性チェックが自動的に実行されます。
...
本実装では、mask パラメータを用いてマルチエージェント学習におけるエージェントの状態表現を切り替えることができます。これは、各エージェントが他のエージェントの存在をどの程度考慮するかを制御するために導入されています。

mask パラメータの値によって、エージェントが Q 学習に使用する状態空間の定義が以下のように変化します。

### mask = 0 **(Cooperative Mode)**
- **使用されるストラテジー**: `CooperativeActionSelection` および `CooperativeQLearning`
- **状態表現**: エージェントは環境の全体状態を観測します。具体的には、Q テーブルの状態 (QState) は 全てのゴール位置 と 全ての（自身を含む）エージェントの位置 をフラット化したタプルとなります。
- **Qテーブルのサイズ**: 状態空間が大きくなるため、Q テーブルのサイズは大きくなります。協調的な行動や学習ロジックを実装するための基盤となります。
### mask = 1 **(Selfish Mode)**
- **使用されるストラテジー**: `SelfishActionSelection` および `SelfishQLearning`
- **状態表現**: 
エージェントは環境の部分的な状態を観測します。具体的には、Q テーブルの状態 (QState) は 全てのゴール位置 と エージェント自身の位置 のみをフラット化したタプルとなります。他のエージェントの位置は状態に含まれません。
- **Qテーブルのサイズ**: 
状態空間が小さくなるため、Q テーブルのサイズは小さくなります。各エージェントが独立して学習する Independent Q-Learning (IQL) の基本的な状態設定に近いです。

このように、mask パラメータを設定することで、異なる状態表現に基づいた学習を容易に切り替えることができ、協調的な設定と自己中心的な設定でのエージェントの振る舞いや学習効率の違いを比較することが可能です。

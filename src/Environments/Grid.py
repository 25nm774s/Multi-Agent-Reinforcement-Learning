...
class Grid:
    ...
    def __init__(self, grid_size: int):
        ...
        self._object_positions: dict[str, PosType] = {} # obj_id -> (x, y)

    def add_object(self, obj_id: str, position: PosType):
        """
        オブジェクトを指定されたグリッド位置に追加します。

        Args:
            obj_id (str): オブジェクトの一意な識別子 (例: 'agent_0', 'goal_1')。
            position (PosType): オブジェクトの (x, y) 座標。
    ...
    def get_object_position(self, obj_id: str) -> PosType:
        """
        オブジェクトの現在の位置を取得します。
    ...
    def get_all_object_positions(self) -> dict[str, PosType]:
        """
        グリッド内の全てのオブジェクトの位置を取得します。

        Returns:
            dict[str, PosType]: オブジェクトIDをその位置にマッピングする辞書。
        """
        #print("get_all_ob...: ",self._object_positions)
        return self._object_positions.copy() # 外部からの変更を防ぐためにコピーを返します。

    def is_position_occupied(self, position: PosType, exclude_obj_id: str = None) -> bool:# type:ignore
        """
        指定された位置が何らかのオブジェクトによって占有されているかチェックします。

        Args:
            position (PosType): チェックする (x, y) 位置。
    ...
    def is_valid_position(self, position: PosType) -> bool:
        """
    ...
    def set_object_position(self, obj_id: str, position: PosType):
        """
    ...
    def sample(self, num_positions:int) -> list[PosType]:
        """
        ...
        available_positions:list[PosType] = [pos for pos in all_possible_positions if pos not in existing_positions_set]
        ...
        positions:list[PosType] = random.sample(available_positions, num_positions)
